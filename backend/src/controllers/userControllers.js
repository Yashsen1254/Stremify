...
export async function sendFriendRequest(req, res) {
    try {
        ...
        if (myId === recipientId) return res.status(400).json({ success: false, message: "You cannot send a friend request to yourself" })

        const recipient = await User.findById(recipientId)

        if (!recipient) return res.status(404).json({ success: false, message: "Recipient user not found" })

        if (recipient.friends.includes(myId)) return res.status(400).json({ success: false, message: "You are already friends with this user" })
